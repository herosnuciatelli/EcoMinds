...
function ProjectCard({
   post
    ...
    return (
        <Card className="max-w-96 hover:border-violet-950 transition-all hover:bg-violet-50">
            <CardHeader>
                ...
                <Link className="group flex flex-col gap-1.5" href={`/project/${post._id}`}>
                    <CardTitle className="text-xl font-bold group-hover:underline">{post.title}</CardTitle>
                    <CardDescription className="line-clamp-4">{post.description}</CardDescription>
                </Link>
